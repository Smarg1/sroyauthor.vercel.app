          ...
          <div className="hidden items-center gap-10 lg:flex">
            {navItems.map(({ href, label }, i) => (
              <Link
                key={label}
                href={href}
                className={`animation-delay-${String(i * 150)} animate-bounce-in text-on-secondary hover:text-on-secondary-container hover:bg-secondary-container rounded-full px-4 py-2 text-2xl font-medium transition-colors duration-300`}
              >
                {label}
              </Link>
            ))}
